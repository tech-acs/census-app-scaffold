<x-app-layout>
    ...
    <div class="flex flex-col max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
        <div class="text-right">
            <a href="{{route('developer.source.create')}}"><x-button>{{ __('Create new') }}</x-button></a>
        </div>
        @if (session('message'))
                        ...
                        @forelse($records as $record)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    @if($record->connection_active)
                                        <a href="{{route('developer.source.connection.test', $record->id)}}" class="text-indigo-600 hover:text-indigo-900">{{ __('Test') }}</a>
                                        <span class="text-gray-400 px-1">|</span>
                                    @endif
                                    <a href="{{route('developer.source.edit', $record->id)}}" class="text-indigo-600 hover:text-indigo-900">{{ __('Edit') }}</a>
                                    <span class="text-gray-400 px-1">|</span>
                                    <form action="{{route('developer.source.destroy', $record->id)}}" method="post" class="inline">
                                        @method('delete')
                                        @csrf
