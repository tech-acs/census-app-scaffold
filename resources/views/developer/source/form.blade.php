        ...
        <div class="grid grid-cols-1 gap-6 pl-3 md:pl-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <x-input id="name" name="name" type="text" class="mt-1 block w-full" value="{{old('name', $source->name ?? null)}}" />
                    <x-input-error for="name" class="mt-2" />
                </div>
                <div>
                    <x-label for="title" value="{{ __('Display title') }} *" />
                    <x-scaffold::multi-lang-input id="title" name="title" value="{{old('title', $source->title ?? null)}}" />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div>
                    ...
                    <x-input id="start_date" name="start_date" type="date" class="mt-1 block w-full" value="{{old('start_date', optional($source ?? null)->start_date?->format('Y-m-d') ?? null)}}" />
                    <x-input-error for="start_date" class="mt-2" />
                </div>
                <div>
                    <x-label for="end_date" value="{!! __('Exercise end date') !!} *" />
                    <x-input id="end_date" name="end_date" type="date" class="mt-1 block w-full" value="{{old('end_date', optional($source ?? null)->end_date?->format('Y-m-d') ?? null)}}" />
        ...
        <div class="pl-3 md:pl-6 space-y-6">
            <div class="grid grid-cols-3 gap-6">
                <div>
                    ...
                    @foreach($databases as $name => $driver)
                        <option value="{{$driver}}" @selected(old('driver', $source->driver ?? false) == $driver)>{{ $name }}</option>
                    @endforeach
                    </select>
                </div>
                <div>
                    <x-label for="host" value="{{ __('Host') }} *" />
                    <x-input id="host" name="host" type="text" class="mt-1 block w-full" value="{{old('host', $source->host ?? null)}}" />
                    <x-input-error for="host" class="mt-2" />
                </div>
                <div>
                    <x-label for="port" value="{{ __('Port') }} *" />
                    <x-input id="port" name="port" type="text" class="mt-1 block w-full" value="{{old('port', $source->port ?? null)}}" />
            ...
            <div class="grid grid-cols-3 gap-6">
                <div>
                    ...
                    <x-input id="database" name="database" type="text" class="mt-1 block w-full" value="{{old('database', $source->database ?? null)}}" />
                    <x-input-error for="database" class="mt-2" />
                </div>
                <div>
                    <x-label for="username" value="{!! __('Username') !!} *" />
                    <x-input id="username" name="username" type="text" class="mt-1 block w-full" value="{{old('username', $source->username ?? null)}}" />
                    <x-input-error for="username" class="mt-2" />
                </div>
                <div>
                    <x-label for="password" value="{{ __('Password') }} *" />
                    <div class="relative mt-1 rounded-md shadow-sm" x-data="{eyeOpener: true}" x-cloak>
                        <input id="password" name="password" x-bind:type="eyeOpener ? 'password' : 'text'" type="password" value="{{old('password', $source->password ?? null)}}" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm block w-full">
            ...
            <div>
                ...
                <select name="connection_active" class="mt-1 block pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                    <option value="1" @selected(old('connection_active', $source->connection_active ?? false) == true)>{{ __('Yes') }}</option>
                    <option value="0" @selected(old('connection_active', $source->connection_active ?? false) == false)>{{ __('No') }}</option>
                </select>
            </div>
        </div>
    </div>
    <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
        <x-secondary-button class="mr-2"><a href="{{ route('developer.source.index') }}">{{ __('Cancel') }}</a></x-secondary-button>
        <x-button>
            {{ __('Submit') }}
        </x-button>
    </div>
